...
var mailOption ={
    from: '"verify your email " <hajji.tas520@gmail.com>',
    to: newUser.email,
    subject: 'vérification your email ',
    html:`<h2>${newUser.firstname}! thank you for registreting on our
    website</h2>
...
router.get('/', async (req, res, )=> {
    try {
    const users = await User.find().select("-password").select("-lastname");
...
router.post('/login', async (req, res) => {
    ...
    if (!email || !password) {
        ...
        const token = jwt.sign({ iduser: user._id, role: user.role
